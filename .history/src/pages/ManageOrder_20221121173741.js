...
const ManageOrder =()=>{
    ...
    toggleInitialState[0] = true;
    const [acceptedState, setAcceptedState] = useState(toggleInitialState);
    const [status, setStatus] = useState("")
    const [record, setRecord] = useState(OrderData);
    //const [switchToggled, setSwitchToggled] = useState(false);     

    const onToogleSwitch = (id) => {
        const toggleCopy = [...acceptedState]
        toggleCopy.fill(false);
        toggleCopy[id] = !setAcceptedState;
        setAcceptedState(toggleCopy);
        console.log(id)
        console.log("From 1:" + acceptedState)
    }
