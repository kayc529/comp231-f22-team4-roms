    ...
    <div className='control-main'>
        ...
        <h2> Status: {orderStatus} </h2>
            ...
            />  */}
                    ...
                    <div onChange={(e) => { setOrderStatus(e.target.value); onUpdateStatus(singleOrder, e.target.value) }} >
                        
                Accept  <input type="radio" value="ACCEPTED" name="radioStatus" defaultChecked={orderStatus}
                    /> 
                Reject <input type="radio" value="REJECTED" name="radioStatus" defaultChecked={orderStatus}
                     />
                Completed <input type="radio" value="COMPLETED" name="radioStatus" defaultChecked={orderStatus}
                     />
                Cancel <input type="radio" value="CANCELED" name="radioStatus" defaultChecked={orderStatus}
                    onChange={(e) => { setOrderStatus(e.target.value); onUpdateStatus(singleOrder, e.target.value) }} />
                Pending <input type="radio" value="PENDING" name="radioStatus" defaultChecked={orderStatus}
                    onChange={(e) => { setOrderStatus(e.target.value); onUpdateStatus(singleOrder, e.target.value);console.log(e.target.value) }} />
