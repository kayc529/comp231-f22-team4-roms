        ...
        <h2> Status: {orderStatus} </h2>
            ...
            />  */}
                    ...
                    {/* <div onChange={(e) =>{setOrderStatus(e.target.value)}} > */}
                        ...
                        Accept  <input type="radio" value="ACCEPTED" name="radioStatus" checked={onUpdateStatus(singleOrder, orderStatus)} onChange={(e) =>{setOrderStatus(e.target.value)}}/> 
                        Reject <input type="radio" value="REJECTED" name="radioStatus" checked={onUpdateStatus(singleOrder, orderStatus)} onChange={(e) =>{setOrderStatus(e.target.value)}}></input>
                        Completed <input type="radio" value="COMPLETED" name="radioStatus" checked={onUpdateStatus(singleOrder, orderStatus)} onChange={(e) =>{setOrderStatus(e.target.value)}}></input>
                        Cancel <input type="radio" value="CANCELED" name="radioStatus" checked={onUpdateStatus(singleOrder, orderStatus)} onChange={(e) =>{setOrderStatus(e.target.value)}}></input>
                Pending <input type="radio" value="PENDING" name="radioStatus" checked={onUpdateStatus(singleOrder, orderStatus)} onChange={(e) => { setOrderStatus(e.target.value); {console.log(e.target.value)}}}></input>
